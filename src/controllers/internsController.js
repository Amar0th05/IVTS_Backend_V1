...
async function createIntern(req, res) {
  ...
  try {
    ...
    request.input("status", sql.Bit, 1);
     request.input("internStatus",  sql.NVarChar, data.source);
    ...
    await request.query(`
      INSERT INTO dbo.internApplicants (
        ...
        BonafideFileData, ResumeFileData, PhotoFileData, IdProofFileData, status,internStatus
      ) VALUES (
        @FullName, @DateOfBirth, @Gender, @OtherGender, @MobileNumber, @CurrentLocation, @Email, @PortfolioLink,
        @EmergencyContactName, @EmergencyContactRelationship, @EmergencyContactNumber,
        @CollegeName, @DegreeProgram, @IsPartOfCurriculum, @FacultySupervisor,
        @PreferredStartDate, @PreferredEndDate, @InternshipMode, @HowHeardAboutUs,
        @BonafideFile, @ResumeFile, @PhotoFile, @IdProofFile, @status @internStatus
      )
    `);

    // Send mail to HR
    await sendHRMail(process.env.HR_CC_EMAIL, data);
  ...
  try {
    ...
    if (data.preferredStartDate !== undefined) {
      ...
      request.input(
        "PreferredStartDate",
        sql.Date,
        parseDateOrNull(data.preferredStartDate)
      );
    }
    ...
    if (data.preferredEndDate !== undefined) {
      ...
      request.input(
        "PreferredEndDate",
        sql.Date,
        parseDateOrNull(data.preferredEndDate)
      );
    }
    ...
    if (data.submissionDate !== undefined) {
      ...
      request.input(
        "SubmissionDate",
        sql.Date,
        parseDateOrNull(data.submissionDate)
      );
    }
    ...
    if (data.certificateStatus !== undefined) {
      updates.push("certificateStatus = @certificateStatus");
      request.input("certificateStatus", sql.NVarChar(50), data.certificateStatus);
    }
...
async function downloadDocument(req, res) {
  ...
  try {
    ...
    if (docName == "Photo") {
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=${docName}.png`
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=${docName}.pdf`
      );
...
async function generateDate(req, res) {
  try {
    ...
    const query = `UPDATE dbo.internApplicants SET ${data.generateName}=CAST(GETDATE() AS DATE) WHERE id = @id`;
...
async function updateInternStatus(req, res) {
  try {
    const { id } = req.params;
    const request = await pool.request();
    ...
    if (result.rowsAffected[0] > 0) {
      ...
      res.status(404).json({ message: "Intern not Approved" });
    }
  } catch (err) {
    console.error("Error Intern Approved:", err);
    res.status(500).json({
      message:
        err.response?.data?.message || err.message || "Internal Server Error",
    });
